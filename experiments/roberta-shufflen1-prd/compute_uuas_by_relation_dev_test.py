#!/usr/bin/env python3
"""
Compute ULAS by dependency relation on dev and test for every trained layer.
Adapted from experiments/bert-base-prd/compute_uuas_by_relation_dev_test.py
for the roberta-shufflen1-prd experiment (13 layers, RobertaDataset).

Usage (from this directory or anywhere):
    python compute_uuas_by_relation_dev_test.py [--layer N]
    (omit --layer to process all 13 layers 0-12)
...
class _DevTestOnlyRobertaDataset(spdata.RobertaDataset):
    """RobertaDataset that loads only dev and test, skipping train."""
    ...
    def read_from_disk(self):
        ...
        dev_obs  = self.optionally_add_embeddings(dev_obs,  dev_emb)
        test_obs = self.optionally_add_embeddings(test_obs, test_emb)
...
def process_layer(layer_idx, results_dir, force=False):
    ...
    # Use the per-layer yaml (has absolute paths already filled in).
    yaml_candidates = glob.glob(os.path.join(layer_result, f'layer-{layer_pad}.yaml'))
    if not yaml_candidates:
        print(f'[layer {layer_pad}] No layer-{layer_pad}.yaml found in {layer_result}; skipping.')
        return
    ...
    dataset    = _DevTestOnlyRobertaDataset(cfg, probe_task)
...
def main():
    ...
    parser.add_argument('--layer', type=int, default=-1,
                        help='Layer index (0-12). Omit to process all layers.')
    parser.add_argument('--force', action='store_true',
                        help='Recompute even if output files already exist.')
    args = parser.parse_args()

    results_dir = os.path.join(SCRIPT_DIR, 'results')
    layers = range(13) if args.layer < 0 else [args.layer]
