...
def report_on_stdin(args):
  ...
  bert_model = HookedEncoder.from_pretrained('bert-large-cased')
  LAYER_COUNT = 24
  ...
  for index, line in tqdm(enumerate(sys.stdin), desc='[demoing]'):
    ...
    tokenized_sent = ['[CLS]'] + tokenizer.tokenize(' '.join(line.strip().split())) + ['[SEP]']
    untok_tok_mapping = data.SubwordDataset.match_tokenized_to_untokenized(tokenized_sent, untokenized_sent)

    indexed_tokens = tokenizer.convert_tokens_to_ids(tokenized_sent)
    segment_ids    = [0] * len(tokenized_sent)

    input_ids      = torch.tensor([indexed_tokens]).to(args['device'])
    token_type_ids = torch.tensor([segment_ids]).to(args['device'])

    with torch.no_grad():
      _, cache = bert_model.run_with_cache(input_ids, token_type_ids=token_type_ids)
      layer_key = f"blocks.{args['model']['model_layer']}.hook_resid_post"
      single_layer_features = cache[layer_key]  # (1, seq, hidden)
