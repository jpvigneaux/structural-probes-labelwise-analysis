...
def choose_task_classes(args):
  ...
  if args['probe']['task_name'] == 'parse-distance':
    ...
    elif args['probe_training']['loss'] == 'ranking':
      loss_class = loss.RankingDistanceLoss
    elif args['probe_training']['loss'] == 'polar':
      loss_class = loss.PolarLoss
    else:
...
def choose_probe_class(args):
  ...
  if args['probe']['task_signature'] == 'word':
    ...
    if args['probe'].get('polar'):
      return probe.PolarProbe
    elif args['probe'].get('orthogonal_projection'):
      return probe.TwoWordOrthogonalProbe
...
def run_train_probe(args, probe, dataset, model, loss, reporter, regimen):
  ...
  """
  report_during_training = args.get('probe_training', {}).get('report_uuas_during_training', False)
  regimen.train_until_convergence(probe, model, loss,
      dataset.get_train_dataloader(), dataset.get_dev_dataloader(),
      reporter=reporter if report_during_training else None)
...
def execute_experiment(args, train_probe, report_results):
  ...
  # PolarProbe requires PolarRegimen (extracts edge vectors for L_A)
  if args['probe'].get('polar'):
    expt_regimen = regimen.PolarRegimen(args)
  else:
...
if __name__ == '__main__':
  ...
  argp.add_argument('--layer',default=-1, type=int,
      help='specify which model layer you want to train')
  argp.add_argument('--rank', default=-1, type=int,
      help='specify the rank of the model')
